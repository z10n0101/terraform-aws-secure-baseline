...
output "config_configuration_recorder" {
  ...
  value = one(module.config_baseline[*].configuration_recorder)
}

output "config_sns_topic" {
  description = "The SNS topic) that AWS Config delivers notifications to."

  value = one(module.config_baseline[*].config_sns_topic)
}
...
output "guardduty_detector" {
  ...
  value = one(module.guardduty_baseline[*].guardduty_detector)
}
...
output "vpc_flow_logs_group" {
  ...
  value = local.flow_logs_to_cw_logs ? one(module.vpc_baseline[*].vpc_flow_logs_group) : null
}

output "default_vpc" {
  description = "The default VPC."

  value = one(module.vpc_baseline[*].default_vpc)
}

output "default_security_group" {
  description = "The ID of the default security group."

  value = one(module.vpc_baseline[*].default_security_group)
}

output "default_network_acl" {
  description = "The default network ACL."

  value = one(module.vpc_baseline[*].default_network_acl)
}

output "default_route_table" {
  description = "The default route table."

  value = one(module.vpc_baseline[*].default_route_table)
}
